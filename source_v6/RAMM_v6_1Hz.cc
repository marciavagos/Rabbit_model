...
{		
	...
	Na_i = 9.1; Na_junc = 9.1; Na_sl = 9.1;
...
void RAMM::Ion_HH_Na(int seg_i, int mem_i, double dt)
{
	...
	double INa_junc_NP = data->Fjunc * data->P_Na * Po_Na1 * Po_Na2 * settings->Na_o * Vm * data->F * data-> frt * (exp((Vm-ENa_junc)*data->frt)-1)/(exp(Vm * data->frt) - 1);
	double INa_sl_NP = (1-data->Fjunc) * data->P_Na * Po_Na1 * Po_Na2 * settings->Na_o * Vm * data->F * data-> frt * (exp((Vm-ENa_sl)*data->frt)-1)/(exp(Vm * data->frt) - 1);
